...
def _system_maestros(m: Maestros) -> list[dict]:
    ...
    contratistas = "\n".join(
        ...
        for c in m.contratistas if c.activo)  # §5.6: un inactivo no se propone por parecido

    def alias_vigente(a) -> bool:
        c = m.contratista(a.valor_canonico) if a.tipo == "contratista" else None
        return c is None or c.activo
